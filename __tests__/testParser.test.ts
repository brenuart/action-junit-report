...
describe('resolvePath', () => {
  ...
  it('should find correct file for Kotlin fileName', async () => {
    ...
    expect(path).toBe(
      'test_results/tests/utils/src/test/java/action/surefire/report/calc/CalcUtilsTest.kt'
    )
  })
})
...
describe('parseFile', () => {
  ...
  it('should parse empty cunit results', async () => {
    const {count, skipped, annotations} = await parseFile(
      'test_results/cunit/testEmpty.xml'
    )
  ...
  it('should parse failure cunit results', async () => {
    const {count, skipped, annotations} = await parseFile(
      'test_results/cunit/testFailure.xml'
    )
  ...
  it('should parse disabled tests', async () => {
    ...
    expect(annotations).toStrictEqual([
      ...
      },
      {
        ...
        message:
          'tests/failed/main.cpp:49: error: check_eq(6, 7): hello world!',
        raw_details: ''
      },
      {
